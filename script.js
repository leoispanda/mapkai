...
function renderPdcFinalRoundPreview(currentRound) {
  ...
  const reenabled = selectPdcFinalReenabledObserver(rewarded, observers, currentRound);
  ...
  try {
    ...
    pdcState.providerDiagnostics = {
      ...
      advancedAudit: {
        ...
        contentDiagnostics: error.contentDiagnostics || null,
      },
    };
  } finally {
...
function selectPdcFinalReenabledObserver(rewarded, observers, currentRound) {
  ...
  const validObservers = observers.filter((observer) => observer?.speakerId && pdcState.observerRosterIds.includes(observer.speakerId));
  if (!validObservers.length) return { skippedReason: "no_valid_archived_perspective" };
  ...
  if (!selected) return { skippedReason: "no_valid_archived_perspective" };
  return {
    ...selected,
...
function renderPdcAdvancedFinalAudit() {
  ...
  return `
    <section class="pdc-advanced-audit">
      ...
      <h2>Advanced Final Audit</h2>
      <p>A higher-level review of stage fit, bias, and decision quality.</p>
...
function renderPdcAdvancedAuditResult(audit) {
  ...
  const version = audit.recommendedVersion || {};
  ...
  return `
    <div class="pdc-advanced-audit-grid">
      ...
      <article>
        <h3>Stage Fit</h3>
        <p>${escapeHtml(bias.stageMismatch || display.summary || "-")}</p>
      </article>
      <article>
        <h3>Overcomplicated Parts</h3>
        ${renderPdcAuditList(bias.overcomplicatedParts)}
      </article>
      <article>
      ...
      <article>
        <h3>Lightweight Now</h3>
        ${renderPdcAuditList(version.lightweightNow)}
      </article>
      <article>
        <h3>Add Later If Traction</h3>
        ${renderPdcAuditList(version.addLaterIfTraction)}
      </article>
      <article>
        <h3>Only For External Partnership</h3>
        ${renderPdcAuditList(version.onlyForExternalPartnership)}
      </article>
      <article>
