...
function renderMap(profile) {
  ...
  logic.innerHTML = `<strong>Learning map:</strong> start small, practice once, review once, then continue.`;
  ...
  getSimplePlan(profile).forEach(([title, description, action, duration], index) => {
    ...
    item.innerHTML = `
      ...
      <span class="node-copy">
        ...
        <span class="node-action">${action}</span>
      </span>
...
function getSimplePlan(profile) {
  ...
  return [
    ["Today", "Learn one basic idea with AI and write it in your own words.", "Ask AI for a simple explanation, then save three key points.", time],
...
emailForm.addEventListener("submit", async (event) => {
  ...
  try {
    await apiPost("/api/auth/start", { email: currentUser.email });
    codeHint.textContent = `We sent a login code to ${currentUser.email}.`;
