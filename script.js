...
const pdcWarmupMessages = [
  { speakerId: "blue-whale", speakerName: "Blue Whale", speakerChineseName: "蓝鲸", role: "Facilitator", text: "Blue Whale is preparing the council..." },
  { speakerId: "ethan-shen", speakerName: "Ethan Shen", speakerChineseName: "沈知衡", role: "Facts & Evidence", text: "Ethan Shen is reviewing evidence..." },
  { speakerId: "clara-lin", speakerName: "Clara Lin", speakerChineseName: "林问心", role: "Emotion & Inner Desire", text: "Clara Lin is checking emotional pressure..." },
  { speakerId: "marcus-lu", speakerName: "Marcus Lu", speakerChineseName: "陆守衡", role: "Risk & Downside", text: "Marcus Lu is stress-testing downside..." },
  { speakerId: "adrian-xu", speakerName: "Adrian Xu", speakerChineseName: "许望远", role: "Opportunity & Long View", text: "Adrian Xu is checking long-term opportunity..." },
];
      ...
      <div class="pdc-live-room">
        <aside class="pdc-roster-panel" aria-label="Council Members">
          ...
          <div class="pdc-roster-list">
            ${activePersonas.map((persona) => renderPdcRosterRow(persona, activeSpeakerId)).join("")}
          </div>
        ...
        <section class="pdc-dialogue-panel" aria-label="Live Council Dialogue">
          <div class="pdc-dialogue-panel-head">
            ...
            ${recap.dialogueProvider && pdcState.founderPreview ? `<span class="pdc-provider-label">${escapeHtml(recap.dialogueProvider)}</span>` : ""}
          </div>
          <div class="pdc-table-topic">
            <span>Decision on the table</span>
...
function renderPdcRosterRow(persona, activeSpeakerId, isObserver = false) {
  const initials = getPersonaInitials(persona);
  const isSelected = pdcState.selectedPersonaId === persona.id;
  const isSpeaking = activeSpeakerId === persona.id;
  return `
    <button class="pdc-roster-row ${isObserver ? "is-observer" : ""} ${isSpeaking ? "is-speaking" : ""} ${isSelected ? "is-selected" : ""}" type="button" data-pdc-persona="${escapeHtml(persona.id)}" aria-pressed="${isSelected}">
...
function renderPdcDialogue(dialogue, activeSpeakerId = "", thinkingLine = null, phaseComplete = true, isWarmupPhase = false) {
  ...
  return `
    <div class="pdc-dialogue-board" aria-label="Live Council Dialogue">
      <ol>
        ${lines.map((line) => `
          <li class="${line.speakerId === activeSpeakerId ? "is-current" : ""}">
...
function renderPdcFounderPhaseDebug(currentRound) {
  ...
  return `
    <p class="pdc-founder-phase-debug">
      ...
      · warmupMode: ${escapeHtml(playbackDebug.warmupMode || "-")} · warmupStartedAt: ${escapeHtml(playbackDebug.warmupStartedAt || "-")} · warmupDurationMs: ${Number(playbackDebug.warmupDurationMs || 0)}
...
function beginPdcWarmup({ phaseLabel = "Round 1A — Position Update", roundNumber = 1, phaseType = "A" } = {}) {
  clearPdcPlaybackTimer();
  const startedAt = Date.now();
  const phase = createPdcWarmupPhase({ phaseLabel, roundNumber, phaseType, startedAt });
  pdcState.warmup = {
    mode: "local",
    startedAt,
    phaseId: phase.id,
    recap: createPdcWarmupRecap(phase),
  };
  pdcState.playback = {
    phaseId: phase.id,
    visibleCount: 0,
...
function getPdcWarmupDebug() {
  ...
  return pdcState.warmupDiagnostics || { warmupMode: "", warmupStartedAt: "", warmupDurationMs: 0 };
}

function createPdcWarmupPhase({ phaseLabel, roundNumber, phaseType, startedAt }) {
  return {
    id: `warmup-${startedAt}`,
    label: phaseLabel,
    phaseLabel,
    ...
    provider: "local",
    isWarmup: true,
    dialogue: pdcWarmupMessages.map((line) => ({
      ...line,
      speakerLocalName: line.speakerChineseName || line.speakerName,
      stanceType: "warmup",
      targetSpeakerId: "",
      targetSpeakerName: "",
      stanceSummary: line.text,
...
function createPdcWarmupRecap(phase) {
  return {
    ...
    councilRoom: {
      ...
      personas: pdcWarmupPersonas,
      sessionRoster: pdcWarmupPersonas,
      rounds: [phase],
    },
  };
}
...
async function continuePdcPhase() {
  ...
  try {
    ...
    if (warmupDiagnostics) {
      ...
      phase.contentDiagnostics = { ...(phase.contentDiagnostics || {}), ...warmupDiagnostics };
...
async function startPdcExperience() {
  ...
  try {
    ...
    if (warmupDiagnostics && data.recap) data.recap.contentDiagnostics = { ...(data.recap.contentDiagnostics || {}), ...warmupDiagnostics };
    const firstRound = data.recap?.councilRoom?.rounds?.[0];
    if (warmupDiagnostics && firstRound) firstRound.contentDiagnostics = { ...(firstRound.contentDiagnostics || {}), ...warmupDiagnostics };
