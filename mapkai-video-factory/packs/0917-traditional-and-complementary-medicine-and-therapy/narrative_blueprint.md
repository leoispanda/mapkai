# Narrative Blueprint: Traditional and complementary medicine and therapy

**Selected engine:** mystery

## Central Narrative Device

A symptom looks like one problem until a careful diagnosis reveals a whole network of causes and care.
...
Begin with a familiar traditional and complementary medicine and therapy phenomenon that becomes puzzling under a careful question. Delay the definition until the puzzle creates a need for the field.
...
The tension is between an intuitive local answer and the broader structure that Traditional and complementary medicine and therapy is designed to reveal.
...
The first turn occurs when the initial explanation fails to account for a consequence, comparison, or boundary case.
...
The aha moment is that the problem is not isolated: it belongs to a system of evidence, constraints, and relationships.
...
Question → investigation → first reversal → expansion into the field's territories → reframing as a connected knowledge map.
...
Let each major branch enter as a response to a distinct part of the central question. Landmark ideas should change the viewer's model, not appear as vocabulary cards.
...
Use diagrams, scale changes, process views, before/after models, and carefully chosen real-world consequences that make the field's reasoning visible.
...
Return to the opening situation and show why it now carries a different meaning.
