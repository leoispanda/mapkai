...
const uiText = {
  en: {
    ...
    homeEyebrow: "MapKAI",
    homeTitle: "MapKAI",
    homeHeadline: "Navigate your knowledge before navigating AI.",
    homeCopy: "Explore your thinking patterns, discover blind spots, and continue deeper reflection in your personal AI.",
    ...
    challengeTitle: "Explore the patterns behind your answers.",
    challengeCopy: "Answer one situation at a time. Your map and reflection become clearer as exploration accumulates.",
    ...
    aboutOpening: "Most people are not lacking information. They are lacking structure.",
    aboutWhatTitle: "What MapKAI Is",
    aboutWhatP1: "MapKAI = Map Knowledge with AI.",
    aboutWhatP2: "MapKAI helps people see knowledge patterns, discover blind spots, connect scattered learning, and generate deeper reflection through AI.",
    ...
    aboutAiP2: "MapKAI helps people navigate themselves before navigating AI.",
    ...
    aboutBoundaryP1: "MapKAI does not try to become your final AI. It generates better reflection inputs for the AI tools you already use.",
  ...
  zh: {
    ...
    homeEyebrow: "MapKAI",
    homeTitle: "MapKAI",
    homeHeadline: "先导航你的知识，再导航 AI。",
    homeCopy: "探索你的思考模式，发现盲点，并把反思带到你的个人 AI 继续深入。",
    homePrimary: "开始探索",
    homeLearning: "查看知识地图",
    homeCategories: "浏览领域",
    coreEyebrow: "核心探索循环",
    ...
    challengeTitle: "探索答案背后的知识模式。",
    challengeCopy: "一次进入一个情境。随着探索积累，你的地图和反思会逐渐清晰。",
    ...
    aboutOpening: "多数人缺少的不是信息，而是结构。",
    aboutWhatTitle: "MapKAI 是什么",
    aboutWhatP1: "MapKAI = Map Knowledge with AI。",
    aboutWhatP2: "MapKAI 帮助人们看见知识模式、发现盲点、连接零散学习，并通过 AI 生成更深的反思入口。",
    aboutAiTitle: "AI 时代的问题",
    aboutAiP1: "AI 可以生成无限答案。但很多人仍然不清楚自己如何思考、反复忽略什么、盲点在哪里。",
    aboutAiP2: "MapKAI 帮助人们先导航自己，再导航 AI。",
    ...
    aboutBoundaryP1: "MapKAI 不试图成为你的最终 AI。它为你已经使用的 AI 工具生成更好的反思输入。",
...
function applyLanguage() {
  ...
  setText(".home-page .hero h2", t("homeHeadline"));
  ...
  target.innerHTML = `
    ...
    ${output ? `<div class="reflection-output">
      <article class="reflection-summary-card">
        <h3>MapKAI Reflection</h3>
        <p class="eyebrow">${t("reflectionSummaryLabel")}</p>
        <p>${escapeHtml(output.summary || "")}</p>
      </article>
      <div class="reflection-briefing-grid">
        <article><h3>${t("primaryPattern")}</h3><p>${escapeHtml(output.primaryPattern || "")}</p></article>
        <article><h3>${t("blindSpot")}</h3><p>${escapeHtml(output.blindSpot || "")}</p></article>
        <article><h3>${t("nextDirection")}</h3><p>${escapeHtml(output.nextDirection || "")}</p></article>
        <article><h3>${t("uncomfortableTruth")}</h3><p>${escapeHtml(output.uncomfortableTruth || "")}</p></article>
      </div>
    ...
    <details class="founder-only reflection-debug">
      ...
      <pre>${escapeHtml(JSON.stringify({
        aiStatus: reflectionApiStatus,
        source: output?.source || "not-generated",
        model: output?.model || "",
        compressedReflectionInput: lastReflectionInput || getCompressedReflectionSummary(),
        metadataDistributions: getAnswerDistributions(),
...
function renderChallenge() {
  ...
  cardTarget.innerHTML = `
    <p class="eyebrow">${t("currentExploration")} -> ${getMasteryLabel(question.unlocksToward)}</p>
    ...
    <div class="challenge-status">${t("exploredPatterns")}: ${getTotalAnsweredQuestions()} · ${t("currentMapState")}: ${getMasteryLabel(getMasteryFromCorrect(activeChallengeSubject))}</div>
    ${renderKnowledgeTitleStatus()}
