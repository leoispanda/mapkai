    ...
    <section class="pdc-council-room" aria-labelledby="pdc-council-room-title">
      ...
      <section class="pdc-blue-whale-screen" aria-label="Blue Whale Summary">
        ...
        <div class="pdc-screen-content">
          ...
          <p>${escapeHtml(facilitator.summary || "Blue Whale has identified the first layer of tension, risks, opportunities, and next-step questions. Live PDC generation will later turn these perspectives into a deeper structured council debate and final decision memo.")}</p>
        </div>
      </section>
      <div class="pdc-meeting-layout">
        <div class="pdc-seat-grid" aria-label="Council participants">
          ${personas.map((persona, index) => renderPdcPersonaCard(persona, index)).join("")}
        </div>
        <section class="pdc-meeting-table" aria-label="Decision on the table">
          <div class="pdc-table-surface">
            ...
            <small>This is the topic the council is reviewing.</small>
          </div>
        </section>
      </div>
    </section>`;
...
function renderPdcPersonaCard(persona, index) {
  return `
    <article class="pdc-persona-card pdc-seat-${index + 1}">
      <div>
        <h3>${escapeHtml(persona.englishName || persona.name || "Council member")}</h3>
        ${persona.name && persona.name !== persona.englishName ? `<p class="pdc-persona-local-name">${escapeHtml(persona.name)}</p>` : ""}
      </div>
      <p class="pdc-persona-role">${escapeHtml(persona.role || "Council participant")}</p>
      <p class="pdc-persona-statement">${escapeHtml(persona.statement || persona.responsibility || "")}</p>
    </article>`;
}
