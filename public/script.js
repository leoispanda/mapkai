...
function renderPdcCouncilRoom(recap) {
  ...
  const activePersona = personas.find((persona) => persona.id === pdcState.activePersonaId) || null;
  return `
    <section class="pdc-council-room" aria-labelledby="pdc-council-room-title">
      <div class="pdc-room-heading">
        ...
        <p>${escapeHtml(room.subtitle || "Your decision is placed on the table. The council reviews it from multiple perspectives.")}</p>
      </div>
      <section class="pdc-blue-whale-screen" aria-label="Blue Whale Summary">
        <div class="pdc-screen-glow"></div>
        <div class="pdc-screen-content">
          <p class="pdc-screen-label">Blue Whale Summary</p>
          <h2>${escapeHtml(facilitator.englishName || "Blue Whale")}</h2>
          ${facilitator.name ? `<p class="pdc-facilitator-name">${escapeHtml(facilitator.name)}</p>` : ""}
          <p>${escapeHtml(facilitator.summary || "Blue Whale is summarizing the first layer of tension, risks, opportunities, and next-step questions. Live PDC generation will later deepen this into a structured council process and final decision memo.")}</p>
        </div>
      </section>
      <div class="pdc-meeting-layout">
        <div class="pdc-seat-grid" aria-label="Council participants">
          ${personas.map((persona, index) => renderPdcPersonaSeat(persona, index)).join("")}
        </div>
        <section class="pdc-meeting-table" aria-label="Decision on the table">
          <div class="pdc-table-surface">
            ...
            ${renderPdcDialogue(room.dialogue || [])}
          </div>
        </section>
      </div>
      ${renderPdcPersonaProfile(activePersona)}
    </section>`;
}

function renderPdcPersonaSeat(persona, index) {
  const initials = getPersonaInitials(persona);
  const isActive = pdcState.activePersonaId === persona.id;
  return `
    <button class="pdc-persona-seat pdc-seat-${index + 1} ${isActive ? "is-active" : ""}" type="button" data-pdc-persona="${escapeHtml(persona.id)}" aria-pressed="${isActive}">
      <span class="pdc-avatar" aria-hidden="true">${escapeHtml(initials)}</span>
      <span class="pdc-seat-copy">
...
function renderPdcDialogue(dialogue) {
  const lines = Array.isArray(dialogue) ? dialogue : [];
  return `
    <div class="pdc-dialogue-board" aria-label="Live Council Dialogue">
      <div class="pdc-dialogue-heading">
        <strong>Live Council Dialogue</strong>
        <span>Current discussion</span>
      </div>
      <ol>
        ${lines.map((line) => `
          <li>
            <span>${escapeHtml(line.speakerName || "Council Member")}${line.speakerLocalName ? ` <small>${escapeHtml(line.speakerLocalName)}</small>` : ""}</span>
            <p>${escapeHtml(line.text || "")}</p>
          </li>`).join("")}
      </ol>
      <p class="pdc-dialogue-note">Placeholder dialogue preview — live PDC generation is not connected yet.</p>
    </div>`;
}
...
function renderPdcRecap(recap) {
  const recapSections = recap.recap || recap.sections || {};
  const sections = [
  ...
  return `
    <section class="pdc-result">
      ...
      <h1>Council Recap</h1>
      ${recap.isPlaceholder ? `<p class="pdc-placeholder-notice">${escapeHtml(recap.placeholderNotice || "Development placeholder output — live PDC API is not connected yet.")}</p>` : ""}
      ...
      <div class="pdc-recap-sections">
        ${sections.map(([title, value]) => `
          <article class="pdc-recap-section">
            <h2>${title}</h2>
            ${Array.isArray(value) ? `<ul>${value.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : `<p>${escapeHtml(value || "")}</p>`}
          </article>`).join("")}
      </div>
    </section>`;
}
...
async function startPdcExperience() {
  if (!pdcState.valid || pdcState.status === "generating") return;
...
document.addEventListener("click", (event) => {
  ...
  if (pdcPersona) {
    ...
    pdcState.activePersonaId = pdcState.activePersonaId === personaId ? "" : personaId;
    renderPdcPilot();
    return;
  }
