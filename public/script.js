...
const uiText = {
  en: {
    ...
    mostActivatedLenses: "Most Activated Lenses",
    occasionallyActivatedLenses: "Occasionally Activated",
    underusedPerspectives: "Underused Perspectives",
    resultSummaryTitle: "Summary",
    continueWithOwnAi: "Continue with your own AI",
  ...
  zh: {
    ...
    mostActivatedLenses: "最常激活的知识视角",
    occasionallyActivatedLenses: "偶尔激活的知识视角",
    underusedPerspectives: "较少调用的视角 / 潜在知识盲区",
    resultSummaryTitle: "简要总结",
    continueWithOwnAi: "复制到你的 AI 继续探索",
    copyPrompt: "一键复制 Prompt",
    copiedPrompt: "已复制",
...
function applyLanguage() {
  ...
  setAllText(".about-page .about-block h2", [
    t("aboutWhatTitle"),
    t("aboutAiTitle"),
    t("aboutExploreTitle"),
  ...
  setAllText(".about-page .about-block p", [
    t("aboutWhatP1"),
    t("aboutWhatP2"),
    t("aboutAiP1"),
    t("aboutAiP2"),
...
function getKnowledgeLensResult() {
  ...
  const mostActivated = ranked.filter((item) => item.count > 0 && item.count >= Math.max(1, topCount - 1)).slice(0, 3);
  const occasionallyActivated = ranked.filter((item) => item.count > 0 && !mostActivated.some((top) => top.code === item.code)).slice(0, 5);
  const underused = ranked.filter((item) => item.count <= 1).sort((left, right) => left.count - right.count || left.code.localeCompare(right.code)).slice(0, 4);
...
function buildLensResultSummary(result = getKnowledgeLensResult()) {
  const topNames = joinLensNames(result.mostActivated);
  const underusedNames = joinLensNames(result.underused);
  if (currentLanguage === "zh") {
    return [
      `你的知识视角更偏向 ${topNames || "多个不同视角"}。你常常从目标、流程、结构、工具、体验或现实约束角度理解问题，也更愿意负责组织、优化、判断和推进类任务。`,
      `较少被激活的视角包括 ${underusedNames || "暂时不明显"}。这不代表弱项，而是提示这些可能是未来值得探索的潜在知识盲区。`,
    ].join("\n\n");
  }
  return [
    `Your knowledge map leans toward ${topNames || "several different lenses"}. You often understand situations through goals, processes, structure, tools, experience, or real-world constraints, and you may prefer tasks involving organization, optimization, judgment, and forward movement.`,
    `Underused perspectives include ${underusedNames || "no clear single area yet"}. This does not mean weakness. It simply marks perspectives that may be worth exploring next.`,
  ].join("\n\n");
}
...
最常激活的知识视角：
${formatLensList(result.mostActivated)}

偶尔激活的知识视角：
${formatLensList(result.occasionallyActivated)}

较少调用的视角 / 潜在知识盲区：
${formatLensList(result.underused)}
...
1. 这可能说明我的哪些知识优势？
2. 我可能有哪些潜在知识盲区？
3. 这种模式可能如何影响我的工作、关系、学习、旅行、消费和日常决策？
4. 我的思考方式可能更接近哪些职业领域或工作方式？
5. 我接下来可以有意识探索哪些知识视角？
6. 我可以采取哪些小行动来扩展较少调用的视角？

重要：
不要把这当成性格测试或预测。
请把它当成基于选择结果的知识结构与行动偏好反思。
请保持客观、细腻、实用，不要绝对化。
不要说这些结果定义了我是谁。`;
  }
...
1. What this may suggest about my knowledge strengths.
2. What potential knowledge blind spots I may have.
3. How this pattern may influence my work, relationships, learning, travel, consumption, and daily decisions.
4. Which professional fields or work styles this thinking pattern may align with.
5. Which knowledge lenses I should intentionally explore next.
6. What small actions I can take to expand my underused perspectives.

Important:
Do not treat this as a personality test or prediction.
Treat it as a knowledge-structure and action-preference reflection based on my choices.
Be objective, nuanced, practical, and non-deterministic.
Avoid exaggerated claims.
Do not say this defines who I am.`;
}
...
function renderLensBucket(title, items, maxCount) {
  return `
    <section class="lens-result-group">
      <h3>${title}</h3>
  ...
  target.innerHTML = `
    ...
    ${result ? `<div class="reflection-output lens-result-output">
      <div class="lens-chart">
        ${renderLensBucket(t("mostActivatedLenses"), result.mostActivated, maxCount)}
        ${renderLensBucket(t("occasionallyActivatedLenses"), result.occasionallyActivated, maxCount)}
        ${renderLensBucket(t("underusedPerspectives"), result.underused, maxCount)}
      </div>
