...
function renderPdcRosterRow(persona, activeSpeakerId) {
  ...
  return `
    <button class="pdc-roster-row ${isSpeaking ? "is-speaking" : ""} ${isSelected ? "is-selected" : ""}" type="button" data-pdc-persona="${escapeHtml(persona.id)}" aria-pressed="${isSelected}">
...
async function continuePdcPhase() {
  ...
  try {
    ...
    phases.push(createNextPdcPlaceholderPhase({ previousPhase: currentPhase, room, userIntervention }));
...
async function requestNextPdcPhase({ previousPhase, room, userIntervention }) {
  ...
  return normalizePdcPhase({ ...data.phase, provider: data.provider, userIntervention }, pdcState.pdcPhases.length, room);
}
